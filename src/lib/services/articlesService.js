...
const listAll = async (options = {}) => {
    ...
    const { category, verdict, sourceId, q, page = 1, limit = 12 } = options;
...
const getCategories = () => {
    const categories = new Set(
        articlesCache.map((a) => a.category).filter(Boolean)
    );
    return Array.from(categories);
};

/**
 * Get unique verdicts from cached articles
 * @returns {Array<string>}
 */
const getVerdicts = () => {
    const verdicts = new Set(
        articlesCache.map((a) => a.verdict).filter(Boolean)
    );
