...
class Orchestrator {
    ...
    generateMockResult(params) {
        ...
        return {
            ...
            agent_reports: agentsUsed.map((agentName) => ({
                agent_name:
                    agentName === "toi"
                        ? "Times of India Agent"
                        : agentName === "ndtv"
                        ? "NDTV Agent"
                        : agentName === "indiaTimes"
                        ? "India Times Agent"
                        : "Generic Verification Agent",
                credibility_score: 70 + Math.floor(Math.random() * 20),
                summary: `Mock verification result for: ${claim.substring(
                    0,
                    50
                )}...`,
                evidence_links: [
                ...
                reasoning:
                    "This is a mock verification result for testing purposes.",
            })),
            source_graph: {
                nodes: [
                    {
                    ...
                    {
                        id: "node_1",
                        url: "https://example.com/amplifier",
                        title: "Amplifier Source",
                        role: "amplifier",
                    },
                ],
                edges: [
            ...
            metadata: {
                ...
                remaining_uncertainties: [
                    "Some details could not be independently verified",
                ],
                test_mode: true,
            },
        };
    ...
    async verify(params) {
        ...
        logger.info("Orchestrator: Starting verification", {
            url,
            source,
            hasText: !!text,
        ...
        if (useTestMode) {
            logger.info(
                "Orchestrator: Running in TEST MODE - returning mock result"
            );
        ...
        try {
            ...
            const aggregatedResult = await openaiService.orchestratorAggregate(
                agentReports,
                reputations
            );
            ...
            const blockchainResult = await blockchainService.storeVerification({
                ...
                timestamp: Date.now(),
                claim: claim,
                accuracyScore: aggregatedResult.confidence || 0.5,
            ...
            const result = {
                ...
                metadata: {
                    ...
                    remaining_uncertainties:
                        aggregatedResult.remaining_uncertainties,
                },
            };
    ...
    async extractClaim(url, text) {
        ...
        if (url) {
            ...
            if (article.content) {
                const claims = await openaiService.extractClaims(
                    article.content
                );
                if (claims.length > 0) {
                    return claims[0].claim;
            ...
            try {
                ...
                allEvidence.unshift({
                    ...
                    domainReputationScore:
                        originalArticle.domainReputationScore,
                    isOriginal: true,
                });
            } catch (error) {
                logger.warn("Failed to fetch original URL", {
                    url: originalUrl,
                    error: error.message,
                });
            }
