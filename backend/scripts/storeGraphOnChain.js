...
Environment Variables Required:
  ...
  BLOCKCHAIN_PRIVATE_KEY   - Private key for signing (not needed for dry-run)
    ...
    if (!/^[a-f0-9]{64}$/i.test(graphHash)) {
        console.error(
            "Error: graph_hash must be a 64-character hexadecimal string"
        );
        process.exit(1);
    }
    ...
    console.log(
        `Mode: ${
            dryRun || process.env.BLOCKCHAIN_DRY_RUN === "true"
                ? "DRY RUN"
                : "LIVE"
        }`
    );
    ...
    try {
        ...
        const result = await blockchainService.storeVerification({
            ...
            timestamp: Date.now(),
            claim: "Manual verification from script",
            accuracyScore: 0.5,
            nodes: [],
            metadata: {
        ...
        if (result.dryRun) {
            ...
            console.log(
                "To submit a real transaction, remove --dry-run flag and set BLOCKCHAIN_DRY_RUN=false"
            );
        } else {
